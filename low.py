...
def low(pi_list_path, search_engine, sample=example_para, criteria=example_criteria, delay=60):
    pi_list = pd.read_csv(pi_list_path)
    ...
    for index, row in pi_list.iterrows():
        ...
        extr_task = f"Provided below is the Google Scholar HTML Table of {pi_name}'s publication list sorted by most recent. From this, pick the 3 most relevant to the criteria: {criteria}. Your response should contain only the publication names listed from 1 to 3."
        ...
        if (index != last_idx):
            print(f"Processing for {pi_name} complete. Waiting for {delay}s and moving to the next")
            sleep(delay)
        else:
